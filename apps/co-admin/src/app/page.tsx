...
const ALLOWED_ROLES: string[] = ["co-admin"];
  ...
  useEffect(() => {
    if (userData) {
      ...
      if (ALLOWED_ROLES.includes(role)) {
        ...
        addToast('Access Denied. This panel is for Co-Admin accounts only.', 'error');
      }
    }
  }, [userData]);
    ...
    <div className={"min-h-screen flex items-center justify-center p-4 bg-gradient-to-br " + "from-slate-900 via-purple-900/20 to-slate-900"}>
      ...
      <div className="w-full max-w-sm relative z-10">
        <div className="text-center mb-8">
          <div className={"inline-flex items-center justify-center w-20 h-20 bg-gradient-to-br " + "from-purple-500 to-violet-500" + " rounded-3xl mb-4 shadow-2xl"}>
            <span className="text-4xl">🛡️</span>
          </div>
          <h1 className="text-2xl font-bold text-white">Smart Bazar Co-Admin</h1>
          <p className="text-slate-400 text-sm mt-1">Co-Admin Panel</p>
        </div>

        <div className={"bg-slate-800/80 backdrop-blur-xl rounded-3xl shadow-2xl border " + "border-purple-500/30" + " overflow-hidden"}>
          <div className="p-8">
            {tab === 'login' && (
              <form onSubmit={handleLogin} className="space-y-5">
                <div>
                  <h2 className="text-xl font-bold text-white">Co-Admin Login</h2>
                  <p className="text-sm text-slate-400 mt-0.5">Sign in with your co-admin credentials</p>
                </div>
                ...
                <div className="flex justify-end">
                  <button type="button" onClick={() => setTab('forgot')} className={"text-xs " + "text-purple-400" + " hover:underline"}>Forgot password?</button>
                </div>
                <button type="submit" disabled={loading} className={"w-full py-3 px-4 rounded-xl font-semibold text-sm text-white bg-gradient-to-r " + "from-purple-500 to-violet-500" + " hover:opacity-90 disabled:opacity-50 transition-all shadow-lg"}>
        ...
        <div className="mt-6 p-6 rounded-3xl bg-slate-800/40 border border-purple-500/20 text-center backdrop-blur-sm animate-fadeIn" style={{ animationDelay: '300ms' }}>
          <p className="text-sm font-semibold text-white mb-3">Become a Smart Partner</p>
          <button 
            onClick={() => window.location.href = getAppUrl('customer') + '/?tab=signup'}
            className="px-6 py-2 bg-gradient-to-r from-purple-500/20 to-violet-500/20 hover:from-purple-500/30 hover:to-violet-500/30 text-purple-300 rounded-full text-xs font-bold transition-all border border-purple-500/30"
          >
            Register Now
          </button>
          <p className="text-[10px] text-slate-500 mt-3 uppercase tracking-widest font-bold">Manager • Store • Delivery</p>
        </div>
