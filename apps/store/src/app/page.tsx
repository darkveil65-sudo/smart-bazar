...
const ALLOWED_ROLES: string[] = ["store"];
...
export default function LoginPage() {
  ...
  useEffect(() => {
    if (userData) {
      ...
      if (role === 'store') {
        window.location.href = window.location.origin + '/dashboard/store';
      } else {
  ...
  return (
    <div className={"min-h-screen flex items-center justify-center p-4 bg-gradient-to-br " + "from-slate-900 via-teal-900/20 to-slate-900"}>
      ...
      <div className="w-full max-w-sm relative z-10">
        <div className="text-center mb-8">
          <div className={"inline-flex items-center justify-center w-20 h-20 bg-gradient-to-br " + "from-teal-500 to-green-500" + " rounded-3xl mb-4 shadow-2xl"}>
            <span className="text-4xl">🏪</span>
          </div>
          <h1 className="text-2xl font-bold text-white">Smart Bazar Store</h1>
          <p className="text-slate-400 text-sm mt-1">Store Management Panel</p>
        </div>

        <div className={"bg-slate-800/80 backdrop-blur-xl rounded-3xl shadow-2xl border " + "border-teal-500/30" + " overflow-hidden"}>
          <div className="p-8">
            {tab === 'login' && (
              <form onSubmit={handleLogin} className="space-y-5">
                <div>
                  <h2 className="text-xl font-bold text-white">Store Login</h2>
                  <p className="text-sm text-slate-400 mt-0.5">Sign in with your store account</p>
                </div>
                ...
                <div className="flex justify-end">
                  <button type="button" onClick={() => setTab('forgot')} className={"text-xs " + "text-teal-400" + " hover:underline"}>Forgot password?</button>
                </div>
                <button type="submit" disabled={loading} className={"w-full py-3 px-4 rounded-xl font-semibold text-sm text-white bg-gradient-to-r " + "from-teal-500 to-green-500" + " hover:opacity-90 disabled:opacity-50 transition-all shadow-lg"}>
        ...
        <div className="mt-6 p-6 rounded-3xl bg-slate-800/40 border border-teal-500/20 text-center backdrop-blur-sm animate-fadeIn" style={{ animationDelay: '300ms' }}>
          <p className="text-sm font-semibold text-white mb-3">Open a Smart Store</p>
          <button 
            onClick={() => window.location.href = getAppUrl('customer') + '/?tab=signup'}
            className="px-6 py-2 bg-gradient-to-r from-teal-500/20 to-emerald-500/20 hover:from-teal-500/30 hover:to-emerald-500/30 text-teal-300 rounded-full text-xs font-bold transition-all border border-teal-500/30"
          >
            Register Store Partner
          </button>
          <p className="text-[10px] text-slate-500 mt-3 uppercase tracking-widest font-bold">Sell • Manage • Grow</p>
        </div>
