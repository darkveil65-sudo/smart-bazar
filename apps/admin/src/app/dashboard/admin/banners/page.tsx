    ...
    <div style={{ padding: 24, maxWidth: 900, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
        <div>
          ...
          <p style={{ fontSize: 14, color: '#64748b', margin: '4px 0 0' }}>Manage the rotating banners on the Customer home screen</p>
        </div>
        <button onClick={openAdd} style={{
          background: 'linear-gradient(135deg,#00a045,#00c853)', color: '#fff',
      ...
      {loading ? (
        ...
        <div style={{ ...card, textAlign: 'center', padding: 60 }}>
          ...
          <p style={{ color: '#94a3b8', fontSize: 13 }}>Add your first hero banner to display on the customer home screen</p>
          <button onClick={openAdd} style={{
            marginTop: 16, background: 'linear-gradient(135deg,#00a045,#00c853)', color: '#fff',
              ...
              <div style={{ flex: 1, padding: '14px 18px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                    ...
                    <span style={{
                      ...
                      color: b.isActive ? '#16a34a' : '#94a3b8',
                    }}>
                      {b.isActive ? '● Active' : '○ Inactive'}
                    </span>
                    <span style={{ fontSize: 10, color: '#94a3b8', fontWeight: 600 }}>Order #{b.order}</span>
                  </div>
                ...
                <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
                  ...
                  <button onClick={() => toggleActive(b)} style={{
                    padding: '6px 14px', borderRadius: 8, fontSize: 12, fontWeight: 700,
                    background: b.isActive ? '#fff7ed' : '#f0fdf4',
                    color: b.isActive ? '#c2410c' : '#16a34a', border: 'none', cursor: 'pointer',
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              <h2 style={{ fontSize: 18, fontWeight: 800, margin: 0, color: '#0a1628' }}>
                {editBanner ? '✏️ Edit Banner' : '+ New Banner'}
              </h2>
              <button onClick={() => setShowForm(false)} style={{ background: '#f1f5f9', border: 'none', borderRadius: 8, padding: '6px 10px', cursor: 'pointer', fontSize: 14 }}>✕</button>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div>
                ...
                <input value={form.headline} onChange={e => setForm(f => ({ ...f, headline: e.target.value }))}
                  placeholder="e.g. Fresh Groceries" maxLength={40}
              ...
              <div>
                ...
                <input value={form.sub} onChange={e => setForm(f => ({ ...f, sub: e.target.value }))}
                  placeholder="e.g. Delivered to your door" maxLength={60}
                  style={{ width: '100%', padding: '10px 14px', borderRadius: 10, border: '1.5px solid #e2e8f0', fontSize: 14, outline: 'none', boxSizing: 'border-box' }} />
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div>
                  <label style={{ fontSize: 12, fontWeight: 700, color: '#374151', display: 'block', marginBottom: 4 }}>Badge Text</label>
                ...
                <div>
                  ...
                  <input value={form.cta} onChange={e => setForm(f => ({ ...f, cta: e.target.value }))}
                    placeholder="Shop Now" maxLength={20}
                    style={{ width: '100%', padding: '10px 14px', borderRadius: 10, border: '1.5px solid #e2e8f0', fontSize: 14, outline: 'none', boxSizing: 'border-box' }} />
                </div>
              </div>
                ...
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  {GRADIENT_PRESETS.map(g => (
                    <button key={g.value} onClick={() => setForm(f => ({ ...f, gradient: g.value }))}
                      title={g.label}
                      style={{
                        width: 36, height: 36, borderRadius: 10, border: form.gradient === g.value ? '3px solid #0a1628' : '2px solid transparent',
                        background: g.value, cursor: 'pointer', outline: 'none',
                        boxShadow: form.gradient === g.value ? '0 0 0 2px #fff inset' : 'none',
                ...
                {imagePreview && (
                  <div style={{ marginTop: 10, display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    <button onClick={() => { setImageFile(null); setImagePreview(''); setForm(f => ({ ...f, imageUrl: '' })); }}
                      style={{ fontSize: 11, color: '#e11d48', background: '#fff1f2', border: 'none', borderRadius: 6, padding: '4px 10px', cursor: 'pointer', fontWeight: 700 }}>
                      Remove
                    </button>
                  </div>
                )}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'flex-end', paddingBottom: 2 }}>
                  <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }}>
                    ...
                    <span style={{ fontSize: 13, fontWeight: 700, color: '#374151' }}>Active (show on app)</span>
                  </label>
                </div>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 24 }}>
              <button onClick={() => setShowForm(false)} style={{
                ...
                background: '#f8fafc', fontSize: 14, fontWeight: 700, cursor: 'pointer', color: '#64748b',
              }}>Cancel</button>
              <button onClick={handleSave} disabled={saving} style={{
                flex: 2, padding: '12px', borderRadius: 12, border: 'none',
                background: saving ? '#94a3b8' : 'linear-gradient(135deg,#00a045,#00c853)',
                color: '#fff', fontSize: 14, fontWeight: 800, cursor: saving ? 'not-allowed' : 'pointer',
                boxShadow: saving ? 'none' : '0 4px 14px rgba(0,200,83,0.35)',
              }}>
