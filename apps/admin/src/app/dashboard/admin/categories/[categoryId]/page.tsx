...
export default function CategoriesUnderStorePage() {
  const { addToast } = useToast();
  const params = useParams();
  const router = useRouter();
  const storeId = params.categoryId as string; // URL param still says categoryId for routing
  ...
  const handleSaveCategory = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      addToast(imageFile ? 'Uploading image and saving...' : 'Saving category...', 'info');
      
      if (editingCategoryId) {
        await categoryService.updateCategory(editingCategoryId, formData.name, imageFile, existingImageUrl);
        addToast('Category updated successfully!', 'success');
      } else {
        const id = `${storeId}-${formData.name.trim().toLowerCase().replace(/[^a-z0-9]+/g, '-')}`;
        await categoryService.addCategory(id, storeId, formData.name, imageFile);
    ...
    <div className="animate-fadeIn">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">Manage categories for store: <span className="font-bold text-foreground">{storeId}</span></p>
        </div>
          ...
          <table className="admin-table w-full text-left">
            <thead>
              <tr className="border-b border-border">
                <th className="py-3 px-4 font-bold text-xs text-slate-500 uppercase tracking-widest">Category</th>
            ...
            <tbody>
              {loading ? (
                Array.from({ length: 4 }).map((_, i) => (
                  <tr key={`cat-skel-${i}`} className="border-b border-border last:border-0">
                ...
                categories.map((c) => (
                  ...
                  <td className="py-4 px-4 text-right">
                    <div className="flex items-center justify-end gap-1">
                      <button 
                        onClick={() => router.push(`/dashboard/admin/subcategories/${c.id}`)} 
                        className="p-2 hover:bg-slate-100 rounded-lg text-slate-500 transition-colors inline-block"
                        title="Manage Sub-Categories"
                      >
                        <svg width="18" height="18" viewBox="0 0 20 20" fill="none"><path d="M10 3a7 7 0 100 14 7 7 0 000-14zM8 7h4M8 10h4M8 13h4" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
                      </button>
                      <button 
                ...
                <tr>
                  <td colSpan={3} className="text-center py-20">
                    <div className="flex flex-col items-center">
                      ...
                      <p className="text-xs text-muted-foreground">Add a new category to organize this store.</p>
                    </div>
                  </td>
                </tr>
              )}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-md animate-fadeIn">
          <form onSubmit={handleSaveCategory} className="w-full max-w-sm bg-card rounded-3xl shadow-2xl border border-border overflow-hidden animate-scaleIn">
            <div className="p-6 border-b border-border flex justify-between items-center">
              <h3 className="text-lg font-extrabold">{editingCategoryId ? 'Edit Category' : 'Add New Category'}</h3>
            ...
            <div className="p-6 space-y-4">
              <div>
                <label className="text-[10px] font-bold text-slate-500 mb-1 block uppercase">Category Name</label>
                <input 
                  type="text" 
                  value={formData.name} 
