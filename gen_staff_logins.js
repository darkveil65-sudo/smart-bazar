...
const staffApps = [
  ...
  },
  {
    ...
    subtitle: 'Co-Admin Panel',
    heading: 'Co-Admin Login',
    description: 'Sign in with your co-admin credentials',
    bg: 'from-slate-900 via-purple-900/20 to-slate-900',
    accentColor: 'from-purple-500 to-violet-500',
    textAccent: 'text-purple-400',
    borderAccent: 'border-purple-500/30',
    allowedRoles: ['co-admin'],
    dashboardPath: '/dashboard/admin',
  },
  {
    appName: 'manager',
    ...
    description: 'Sign in with your manager credentials',
  ...
  },
  {
    ...
    heading: 'Store Login',
    description: 'Sign in with your store account',
    bg: 'from-slate-900 via-teal-900/20 to-slate-900',
  ...
  },
  {
    ...
    heading: 'Delivery Login',
    description: 'Sign in with your delivery account',
    bg: 'from-slate-900 via-emerald-900/20 to-slate-900',
...
const generateStaffLoginPage = (cfg) => {
  return `'use client';
...
const ALLOWED_ROLES: string[] = ${JSON.stringify(cfg.allowedRoles)};
...
export default function LoginPage() {
  ...
  useEffect(() => {
    if (userData) {
      ...
      if (ALLOWED_ROLES.includes(role)) {
        window.location.href = window.location.origin + '${cfg.dashboardPath}';
      } else {
        // Wrong role — sign them out and display an error
        clientAuth.signOut();
        useAuthStore.setState({ user: null, userData: null });
        document.cookie = 'userRole=; path=/; max-age=0';
        addToast('Access Denied. This panel is for ${cfg.heading.replace(' Login', '')} accounts only.', 'error');
      }
    }
  }, [userData]);
  ...
  return (
    <div className={"min-h-screen flex items-center justify-center p-4 bg-gradient-to-br " + "${cfg.bg}"}>
      ...
      <div className="w-full max-w-sm relative z-10">
        <div className="text-center mb-8">
          <div className={"inline-flex items-center justify-center w-20 h-20 bg-gradient-to-br " + "${cfg.accentColor}" + " rounded-3xl mb-4 shadow-2xl"}>
        ...
        <div className={"bg-slate-800/80 backdrop-blur-xl rounded-3xl shadow-2xl border " + "${cfg.borderAccent}" + " overflow-hidden"}>
          <div className="p-8">
            {tab === 'login' && (
              <form onSubmit={handleLogin} className="space-y-5">
                <div>
                  <h2 className="text-xl font-bold text-white">${cfg.heading}</h2>
                ...
                <div className="flex justify-end">
                  <button type="button" onClick={() => setTab('forgot')} className={"text-xs " + "${cfg.textAccent}" + " hover:underline"}>Forgot password?</button>
                </div>
                <button type="submit" disabled={loading} className={"w-full py-3 px-4 rounded-xl font-semibold text-sm text-white bg-gradient-to-r " + "${cfg.accentColor}" + " hover:opacity-90 disabled:opacity-50 transition-all shadow-lg"}>
        ...
        <p className="text-center text-xs text-slate-600 mt-6">
          Staff accounts are managed by the system administrator.
        </p>
      </div>
    </div>
  );
...
staffApps.forEach(cfg => {
  const filePath = path.join('apps', cfg.appName, 'src', 'app', 'page.tsx');
  if (!fs.existsSync(path.join('apps', cfg.appName))) {
    console.log('Skipping ' + cfg.appName + ' (dir not found)');
    return;
  }
  fs.writeFileSync(filePath, generateStaffLoginPage(cfg), 'utf8');
  console.log('✅ ' + cfg.appName + ' login page updated');
});
